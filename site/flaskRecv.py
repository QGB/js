...
def action():
	n=U.ct(gaction)
	r=make_response( '\n'.join([i[0] for i in gaction][n*200:(n+1)*200])  )
...
key=DRIVER+r'\QGB\software\xxnet\data\gae_proxy\Certkey.pem'
crt=DRIVER+r'\QGB\software\xxnet\data\gae_proxy\certs\%s.crt'%gsName
